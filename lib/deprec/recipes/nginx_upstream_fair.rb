...
Capistrano::Configuration.instance(:must_exist).load do 
  namespace :deprec do 
    namespace :nginx_upstream_fair do
      ...
      SRC_PACKAGES[:nginx] = {
        ...
        :configure => "./configure --sbin-path=/usr/local/sbin --with-http_ssl_module --add-module=#{File.join(src_dir, 'nginx-upstream-fair.git')};"
      }

      SRC_PACKAGES[:nginx_upstream_fair] = {
        :url => "git://git.localdomain.pl/nginx-upstream-fair.git",
        :download_method => :git,
      }
      ...
      task :download_upstream_fair, :roles => :web do
        deprec2.download_src(SRC_PACKAGES[:nginx_upstream_fair], src_dir)
      end
